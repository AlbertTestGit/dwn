...
foreach($res as $item) {
    ...
    $product = get_page_by_title( $pluginName, OBJECT, 'product' );
    $permUrl = get_permalink( $product->ID );
    $unused = ($item->unused);
    $total = ($item->total);
  ?>
    <div class='pluginSector'>
        <ul >
            <li class='pluginName'><a class="permLink" href="<?php   echo $permUrl; ?>"><?php echo $pluginName;
             ?></a></li>
